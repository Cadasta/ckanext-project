...
class MigrationCommand(CkanCommand):
    '''
    CKAN 'Related Items' to 'project' migration command.

    Usage::

        paster project migrate -c <path to config file>
            - Migrate Related Items to projects

    Must be run from the ckanext-project directory.
    '''
...
migration can continue. Please correct and try again:"""
        ...
        for related in related_items:
            existing_project = get_action('package_search')(data_dict={'fq': '+dataset_type:project original_related_item_id:{0}'.format(related['id'])})
            normalized_title = substitute_ascii_equivalents(related['title'])
            if existing_project['count'] > 0:
                print('project for Related Item "{0}" already exists.'.format(normalized_title))
            else:
                data_dict = {
                ...
                # make the project
                try:
                    new_project = get_action('ckanext_project_create')(data_dict=data_dict)
                except Exception as e:
                    print('There was a problem migrating "{0}": {1}'.format(normalized_title, e))
                else:
                    print('Created project from the Related Item "{0}"'.format(normalized_title))

                    # make the project_package_association, if needed
                    try:
                        related_pkg_id = self._get_related_dataset(related['id'])
                        if related_pkg_id:
                            get_action('ckanext_project_package_association_create')(data_dict={'project_id': new_project['id'],
                                                                                                 'package_id': related_pkg_id})
                    except Exception as e:
                        print('There was a problem creating the project_package_association for "{0}": {1}'.format(normalized_title, e))
