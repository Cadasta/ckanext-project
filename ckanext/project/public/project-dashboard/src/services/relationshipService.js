var app = angular.module("app")
    .service("relationshipService", ['$http', '$q', 'ENV', function($http, $q, ENV) {
         ...
         * @returns {*}
         */
        service.getProjectRelationshipsList = function(projectId, limit, offset) {

            var deferred = $q.defer();

            $http.get(ENV.apiCKANRoot + '/cadasta_get_project_relationship_list?project_id=' + projectId + '&limit=' + limit + '&offset=' + offset, {
                cache: false
            }).
            then(function(response) {
                if (response.data && response.data.error) {
